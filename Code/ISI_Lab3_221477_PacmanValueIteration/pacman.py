...
from agent_ValueIteration import agent_ValueIteration
...
class Pacman:
    ...
    def __init_all_states(self):
        ...
        food_positions = [x for x in food_positions if x]  # remove empty
    ...
    def step(self, action):
        ...
        return self.get_state(), reward, len(self.foods) == 0, self.score
...
# Training the agent
agent = agent_ValueIteration(pacman, 0.9, 0.001)
...
while not done:
    ...
    state, reward, done, score = pacman.step(agent.get_action_for_state(state))
    print(score)
    clock.tick(2)
